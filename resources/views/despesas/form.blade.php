...
<div class="rounded text-center p-3 bg-white shadow-lg">
    ...
    <form action="{{isset($despesa) ? route('despesas.update', $despesa['id_despesa']) : route('despesas.store')}}" method="POST" style="width: 30rem;">
        @csrf
        @isset($despesa)
        @method('put')
        @endisset
        <div class="input-group mb-3">
            ...
            <input type="text" name="descricao" class="form-control" placeholder="Descrição do Gasto" value="{{isset($despesa) ? $despesa['descricao'] : ''}}">
        </div>
        <div class="input-group mb-3">
            ...
            <select name="dia_vencimento" class="form-select">
                ...
                @for($i = 1; $i <= 31; $i++) <option value="{{$i}}" {{isset($despesa) ? ($despesa['dia_vencimento'] == $i ? 'selected' : ''): ''}}>{{$i}}</option>
                    @endfor
            </select>
        </div>
        <div class="input-group mb-3">
            <span class="input-group-text">Valor</span>
            <input type="text" name="valor" class="form-control money" placeholder="Valor" value="{{isset($despesa) ? $despesa['valor'] : ''}}">
        </div>
        <div class="input-group mb-3">

            ...
            @if(!empty($despesas))
                @foreach($despesas as $d)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('despesas.edit', $d['id_despesa']) }}"
                               class="btn btn-sm btn-warning rounded-pill text-white">
                                <i class="fa fa-edit"></i>
                                Editar
                            </a>
                            <a href="{{ route('despesas.index') }}" class="btn btn-sm btn-danger rounded-pill"
                               onclick="event.preventDefault();document.querySelector('#deleteDespesa{{ $d['id_despesa'] }}').submit()">
                                <i class="fa fa-trash"></i>
                                Remover
                            </a>
                            <form action="{{ route('despesas.destroy', $d['id_despesa']) }}" method="POST"
                                  id="deleteDespesa{{ $d['id_despesa'] }}">@csrf @method('delete')</form>
                        </td>
                    </tr>
                @endforeach
            @else
